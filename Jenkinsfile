...
// trigger job every sunday at 0:30
String cron_string = (BRANCH_NAME == "master")? "30 0 * * 0" : ""
...
//        }
    ...
    stage('Build site') {
      ...
      steps {
        ...
        bundle install
        bundle update
        export ASSET_HOST_URL=https://github.dxc.com
    ...
    stage('HTML Proofer') {
      ...
      steps {
        ...
        sh  'htmlproofer --allow-hash-href --only-4xx  --http-status-ignore "429" --empty-alt-ignore --url-ignore /edit\\/master/,/issues\\/new/,/jenkins.platformdxc.com/,/\\/apis$/,/\\/use_cases$/ ./_site'
